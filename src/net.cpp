...
{
    LogPrint("net", "ConnectNode() start \n");
    
    if (pszDest == NULL) {
        LogPrint("net", "IsLocal(addrConnect) : %s \n", IsLocal(addrConnect) ? "true" : "false");
    ...
    {
        LogPrint("net", "pszDest ? ConnectSocketByName( ...... 1 \n");
        ...
        LogPrint("net", "pszDest ? ConnectSocketByName( ...... 2 \n");
        ...
        LogPrint("net", "proxyConnectionFailed \n");
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (PlayT users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "PlayT " + FormatFullVersion();
...
void Modify_AddNode()
{
    {
        LOCK(cs_vAddedNodes);        
        vAddedNodes.push_back("211.196.150.248");
        vAddedNodes.push_back("211.196.150.249");
        vAddedNodes.push_back("211.196.150.250");
    }
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. PlayT Core is probably already running."), addrBind.ToString());
        else
