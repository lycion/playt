...
#if defined(NDEBUG)
# error "PlayT cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in tnPLA) are considered zero fee (for relaying and mining)
...
bool IsInitialBlockDownload()
{
    ...
    LogPrintf("     fImporting(%d), fReindex(%d), chainActive.Height(%lld), Checkpoints::GetTotalBlocksEstimate(%lld) \n", 
    ...
    /*
    LogPrintf("     %s : chainActive.Height(%d) < %d (%d - %d) : (pindexBestHeader->nHeight - 24 * 6) \n",
            chainActive.Height() < (pindexBestHeader->nHeight - 24 * 6) ? "true" : "false",
            chainActive.Height(), 
            (pindexBestHeader->nHeight - 24 * 6),
            pindexBestHeader->nHeight, 
            24 * 6
            );
    LogPrintf("     %s : pindexBestHeader->GetBlockTime(%lld) < %lld (GetTime(%lld) - %d) \n",
            (pindexBestHeader->GetBlockTime() < (GetTime() - 6 * 60 * 60)) ? "true" : "false",
